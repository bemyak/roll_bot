...
pub fn tg_encode(msg: &str) -> String {
	let char_regex: Regex = Regex::new(r"%([\dA-F]{2})").unwrap();
...
pub fn tg_decode(msg: &str) -> String {
	let char_regex: Regex = Regex::new(r"_([\dA-F]{2})_").unwrap();
...
pub fn zalgofy(text: &mut String) {
	let options: GeneratorArgs = GeneratorArgs::new(true, true, true, ZalgoSize::None);
